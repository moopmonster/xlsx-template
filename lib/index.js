...
module.exports = (function() {
    ...
    */
    ...
    Workbook.prototype.substitute = function(sheetName, substitutions) {
        ...
        sheetData.findall("row").forEach(function(row,rowIndex,rowArray) {

            row.attrib.r = currentRow = self.getCurrentRow(row, totalRowsInserted);
            if(row.attrib.r <= 65535)
                rows.push(row);
            ...
            var cells = [],
                ...
                replace_indicator = [], // to indicate if the new table row is to replace or treated as a new row.
		        skipAppend = false;

            row.findall("c").forEach(function(cell,rowI) {
                var appendCell = true && !skipAppend;
                ...
                if(cell.attrib.t === "s") {
                    ...
                    self.extractPlaceholders(string).forEach(function(placeholder) {
                        ...
                        // ftable is fixed table, based on fixed data. 
                        // ctable is like table, but with 'replacement' on 'c'olumn, array data
                        if(placeholder.full && ["table","ftable","ctable"].includes(placeholder.type) && substitution instanceof Array) {
                            let table_replace = placeholder.type === "ftable";
                            let column_replace = placeholder.type === "ctable";
                            let replace_rowRef= []; // for ftable replacement, send in all rows for reference and cell copying
                            if(table_replace) sheetData.findall("row").forEach((o,i)=>{ if(i>rowIndex) replace_rowRef.push(o) });
                            //else if (self.option.replaceExistingCell) 
                            ...
                            newCellsInserted = self.substituteTable(
                                ...
                                placeholder, drawing, table_replace, replace_rowRef, replace_indicator, column_replace
                            );
            ...
            if(newTableRows.length > 0) {
                ...
                var totalPushDown = 0;
                newTableRows.forEach(function(row,i) {
                    ...
                    rows.push(row);
                    if(replace_indicator[i] === false) {
                        ++totalRowsInserted;
                        ++totalPushDown;
                    }
                });

                self.pushDown(self.workbook, sheet.root, namedTables, currentRow, totalPushDown);
            }
      ...
      // existing as well as just substituted
        ...
        // extract ${x} from workbook tabs. 
     
        const regex = /\$\{([^}]+)\}/gi;
        let str = etree.tostring(self.workbook);
        let m;
        ...
        while ((m = regex.exec(str)) !== null) {
            ...
            m.forEach((match, groupIndex) => {
                ...
                str = str.replace(m[0],substitutions[m[1]])
            });
        }
        self.archive.file(self.workbookPath, str);
    ...
    Workbook.prototype.substituteArray = function(cells, cell, substitution, replace_cell, _curRow, column_replace, fromMain) {
        var self = this;
        if(replace_cell == null) replace_cell = false;
        ...
        substitution.forEach(function(element) {
            ...
            if(newCellsInserted > 0) {
                ...
                if(replace_cell || column_replace) {
                    if(currentCell.match(/[A-Z]+/) != null) {
                        nextCellIndex = self.charToNum(currentCell.match(/[A-Z]+/)[0] );
                        nextCell =  _curRow.findall("c")[nextCellIndex-1];
                    } else {
                        nextCell = cell;
                    }
                }
            ...
            var newCell = self.cloneElement(nextCell);
            if(newCell._children.length === 0) {
                newCell._children = self.cloneElement(oriCellRef,true)._children
            }
            self.insertCellValue(newCell, element);
            if(!replace_cell || fromMain) 
    ...
    Workbook.prototype.substituteTable = function(row, newTableRows, cells, cell, namedTables, substitution, key, placeholder, drawing, table_replace, rows, replace_indicator, column_replace) {
        var self = this,
            ...
            substitution.forEach(function(element, idx) {
                ...
                if(idx === 0) { // insert in the row where the placeholders are
                    ...
                    if(value instanceof Array) {
                        newCellsInserted = self.substituteArray(cells, cell, value, table_replace, row, column_replace,false);
                    ...
                    let cellIndex = self.charToNum(cell.attrib.r.match(/[A-Z]+/)[0]) // table_replace? self.charToNum(cell.attrib.r.match(/[A-Z]+/)[0]):0;
                    ...
                    if((idx - 1) < newTableRows.length) {
                        ...
                        if(!table_replace) {
                            ...
                            // clone the next row. 
                            newRow = self.cloneElement(rows[idx-1]);
                        }
                        newTableRows.push(newRow);
                        replace_indicator.push(table_replace);
                    }
                    ...
                    if(!table_replace) {
                        if(!column_replace) {
                            ...
                            newCell = self.cloneElement(row.findall("c")[cellIndex-1]);
                            newCell.attrib.r = self.joinRef({
                                row: newRow.attrib.r,
                                col: self.splitRef(newCell.attrib.r).col
                            });
                        ...
                        newCell = self.cloneElement(rows[idx-1].findall("c")[cellIndex-1]);
                    }

                    if(newCell._children.length === 0) {
                        newCell._children = self.cloneElement(cell,true)._children
                    }

                    if(value instanceof Array) {
                        let tmp_rows = table_replace? rows[idx-1] : row;                            
                        newCellsInsertedOnNewRow = self.substituteArray(newCells, newCell, value, table_replace, tmp_rows, column_replace,false);

                        // Add each of the new cells created by substituteArray()
                        newCells.forEach(function(cellObj) {
                            newRow.append(cellObj);
                        });
        ...
        return (table_replace? 0:newCellsInserted);
    };
